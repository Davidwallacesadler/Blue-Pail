...
struct NavigationBarHelper {
    ...
    static func setupDarkModeNavigationBar(viewController: UIViewController) {
        viewController.navigationController?.navigationBar.barTintColor = UIColor.darkGrayBlue
        viewController.navigationController?.navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.mintGreen, NSAttributedString.Key.font: UIFont(name: "AvenirNext-Medium", size: 18)!]
    }
}
